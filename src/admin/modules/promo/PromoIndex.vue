<template>
  <div class="flex flex-col gap-6">
    ...
    <template v-if="tab === 'records'">
      ...
      <!-- Release filter -->
      <div class="flex flex-col gap-1.5">
        <label class="font-mono text-[0.55rem] tracking-[0.2em] uppercase text-fg-dim">Filter by release</label>
        <select v-model="recordsReleaseFilter" @change="loadRecords" class="tmr-input w-full sm:max-w-xs">
          <option value="">— all releases —</option>
          <option v-for="r in releases" :key="r.slug" :value="r.slug">
            {{ r.catalog }} — {{ r.title }} ({{ r.artist }})
          </option>
        </select>
      </div>
      ...
      <!-- Add Record form -->
      <div v-if="showAddRecord" class="border border-border p-5 flex flex-col gap-4">
        <span class="font-mono text-[0.6rem] tracking-[0.3em] uppercase text-accent">// ADD_RECORD</span>
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div class="flex flex-col gap-1.5">
            <label class="font-mono text-[0.55rem] tracking-[0.2em] uppercase text-fg-dim">Release *</label>
            <select v-model="newRecord.release_slug" class="tmr-input w-full">
              <option value="">— select release —</option>
              <option v-for="r in releases" :key="r.slug" :value="r.slug">
                {{ r.catalog }} — {{ r.title }} ({{ r.artist }})
              </option>
          ...
          <div class="flex flex-col gap-1.5">
            <label class="font-mono text-[0.55rem] tracking-[0.2em] uppercase text-fg-dim">Recipient Name *</label>
            <input v-model="newRecord.recipient_name" type="text" class="tmr-input w-full" placeholder="Press / Blog / Person" />
          </div>
          <div class="flex flex-col gap-1.5">
            <label class="font-mono text-[0.55rem] tracking-[0.2em] uppercase text-fg-dim">Email (optional)</label>
            <input v-model="newRecord.recipient_email" type="email" class="tmr-input w-full" placeholder="recipient@example.com" />
          </div>
          <div class="flex flex-col gap-1.5">
            <label class="font-mono text-[0.55rem] tracking-[0.2em] uppercase text-fg-dim">Expires at (optional)</label>
            <input v-model="newRecord.expires_at" type="date" class="tmr-input w-full" />
          </div>
        </div>
        <div class="flex items-center gap-3 flex-wrap">
          <button
            @click="addRecord"
            :disabled="addingRecord || !newRecord.release_slug || !newRecord.recipient_name.trim()"
            class="btn btn--primary"
          >{{ addingRecord ? 'Saving…' : 'Save' }}</button>
          ...
          <div class="flex-1 min-w-0 flex flex-col gap-1.5">
            ...
            <div class="flex items-center gap-2 min-w-0">
              <a
                ...
                target="_blank"
                rel="noopener"
                class="font-mono text-[0.5rem] tracking-[0.1em] text-fg-dim hover:text-fg truncate max-w-[22rem]"
                :title="promoUrl(rec)"
              >{{ promoUrl(rec) }}</a>
              <button
                @click="copyUrl(rec)"
            ...
            <!-- Meta -->
            <span class="font-mono text-[0.45rem] tracking-[0.15em] uppercase text-fg-dim">
              Created: {{ rec.created ? new Date(rec.created.replace(' ', 'T')).toLocaleDateString('de-DE') : '—' }}
              <template v-if="rec.expires_at"> · Expires: {{ new Date(rec.expires_at.replace(' ', 'T')).toLocaleDateString('de-DE') }}</template>
            </span>
          </div>
          <button
...
import { promoApi, promoRecordsApi, type PromoSubscriber, type PromoRecord } from '../../../lib/admin-api';
...
// ── Tab ──────────────────────────────────────────────────────────────────────
const tab = ref<'list' | 'records'>('list');
...
// ── Promo Records state ───────────────────────────────────────────────────────
const records              = ref<PromoRecord[]>([]);
const recordsLoading       = ref(false);
const recordsError         = ref('');
const recordsReleaseFilter = ref('');
...
async function loadRecords() {
  ...
  recordsError.value = '';
  try {
    records.value = await promoRecordsApi.getRecords(recordsReleaseFilter.value || undefined);
  } catch (e: any) {
    recordsError.value = e.message ?? 'Failed to load records.';
  } finally {
...
async function addRecord() {
  if (!newRecord.value.release_slug || !newRecord.value.recipient_name.trim()) return;
  addingRecord.value = true;
  addRecordError.value = '';
  try {
    const rec = await promoRecordsApi.createRecord({
      release_slug: newRecord.value.release_slug,
      recipient_name: newRecord.value.recipient_name.trim(),
      ...(newRecord.value.recipient_email ? { recipient_email: newRecord.value.recipient_email } : {}),
...
function cancelAddRecord() {
  ...
  addRecordError.value = '';
  newRecord.value = { release_slug: recordsReleaseFilter.value, recipient_name: '', recipient_email: '', expires_at: '' };
}

// ── Promo Records: Delete ─────────────────────────────────────────────────────
async function removeRecord(id: string) {
  if (!confirm('Delete this promo record? The token will stop working.')) return;
  deletingRecordId.value = id;
  try {
    await promoRecordsApi.deleteRecord(id);
...
// ── Promo Records: URL + copy ─────────────────────────────────────────────────
function promoUrl(rec: PromoRecord): string {
  return `https://the-moon-records.de/promo/${rec.release_slug}?t=${rec.token}`;
}

async function copyUrl(rec: PromoRecord) {
  try {
    await navigator.clipboard.writeText(promoUrl(rec));
    copiedIds.value = new Set(copiedIds.value).add(rec.id);
    setTimeout(() => {
