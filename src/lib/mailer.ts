...
export async function sendSplitSigningEmail(
  to: string,
  artistName: string,
  releaseTitle: string,
  percentage: number,
  ...
  const html = baseHtml(`
    <h1 style="margin:0 0 16px;color:#E8E4D8;font-size:22px;letter-spacing:0.2em;text-transform:uppercase;font-weight:700;">
      Split Sheet — Please Sign
    </h1>
    <p style="margin:0 0 24px;color:rgba(232,228,216,0.6);font-size:13px;letter-spacing:0.08em;line-height:1.8;">
      Hi ${name}, your split sheet for &ldquo;${title}&rdquo; is ready — your share is ${percentage}%.
  ...
  const text = `Hi ${artistName},\n\nYour split sheet for "${releaseTitle}" is ready — your share is ${percentage}%.\n\nReview and sign:\n${signingUrl}\n\nThis link is personal to you and can only be used once.`;
